...
inline __host__ __device__ vec3 vec3_negate(vec3 v) { return make_vec3(-v.x, -v.y, -v.z); }
...
inline __host__ __device__ vec3 unit_vector(vec3 v) { return v / vec3_length(v); }

inline __host__ __device__ vec3 reflect_vector(vec3 vec, vec3 normal)
{
