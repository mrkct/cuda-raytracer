...
static __device__ float dielectric_reflectance(float const cosine, float const ref_idx)
{
...
{
    float const cos_theta = my_fmin(vec3_dot(vec3_negate(uv), n), 1.0);
    vec3 const r_out_perp = etai_over_etat * (uv + cos_theta * n);
    vec3 const r_out_parallel = -sqrt(fabs(1.0 - vec3_length_squared(r_out_perp))) * n;

    return r_out_perp + r_out_parallel;
}

__device__ bool dielectric_scatter(void* vdata, curandState_t* rng_state, struct Ray const& ray, HitRecord* rec,
    color* out_attenuation, struct Ray* out_scattered)
{
    ...
    *out_attenuation = make_color(1.0, 1.0, 1.0);
    float const refraction_ratio = rec->front_face ? (1.0 / data->refraction_level) : data->refraction_level;

    const vec3 unit_direction = unit_vector(ray.direction);

    float const cos_theta = my_fmin(vec3_dot(vec3_negate(unit_direction), rec->normal), 1.0);
